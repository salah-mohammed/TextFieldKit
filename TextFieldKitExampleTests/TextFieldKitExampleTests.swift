...
final class TextFieldKitExampleTests: XCTestCase {
    ...
    let field0 = Validate.required(forthViewController.vmFullName.title)
    let field1 = Validate.required(forthViewController.vmPasswordField.title)
    let field2 = Validate.required(forthViewController.vmNewPassword.title)
    let field3 = Validate.required(forthViewController.vmConfirmPassword.title)
    ...
    let field0 = AppTexts.Constant.passwordNotMatch.string
    XCTAssertEqual(valueCheck(errors[0].message,field0),true)
    }
